...
pipeline {
  agent {
    kubernetes { // 쿠버네티스 pod를 젠킨스 작업이 수행하는 에이전트로 사용, yaml으로 정의
      yaml ''' // 여기서부터 pod 명세
      apiVersion: v1
      kind: Pod
      metadata:
      ...
      spec:
        containers:
        - name: kustomize // kustomize가 설치된 컨테이너를 에이전트 pod에 포함
          ...
          command:
          - cat
        serviceAccount: jenkins // 에이전트 pod이 쿠버네티스 클러스터에 오브젝트를 배포하기 위해 사용할 서비스 어카운트
        volumes:
        - name: kubectl
          hostPath:
            path: /bin/kubectl
  ...
  stages { // 깃헙으로부터 소스 코드 내려 받는 단계
    stage('git scm update'){
      steps {
        git url: 'https://github.com/IaC-Source/blue-green.git', branch: 'main'
      }
    }
    stage('define tag'){ // 빌드 횟수마다 부여되는 번호에 따라 블루, 그린 전환 표시
      steps {
        script {
    ...
    stage('deploy configmap and deployment'){ // configmap 배포한 다음 디플로이먼트 배포
      steps {
        container('kustomize'){
          dir('deployment'){ // 배포를 위한 yaml파일이 깃헙 하위 디플로이먼트 디렉토리에 있어서 지정
            sh '''
            kubectl apply -f configmap.yaml
            kustomize create --resources ./deployment.yaml
            echo "deploy new deployment"
            kustomize edit add label deploy:$tag -f // tag 환경변수 일일이 수정하지 않기 위해 kustomize 명령 사용
    ...
    stage('switching LB'){ // 배포 후 로드밸런서에 보내줄 대상 설정
      steps {
        container('kustomize'){
          dir('service'){ // 로드밸런서 관련 yaml 파일의 깃헙 디렉토리 설정
            sh '''
...
1. Build Now 버튼을 눌러 배포 확인
2. 슈퍼 푸티 창으로 돌아가 정상 배포 확인
    
    ```sql
    kubectl get deployments, service --selector=app=dashboard
    ```
    
3. 호스트 노트북의 브라우저에서 192.168.1.12 입력해 배포된 파란색 대시보드 확인
4. 두 번째 배포 진행 - Build now 버튼 클릭
5. 슈퍼 푸티에서 배포 확인하면 pl-blue 디플로이먼트 삭제되는 것 확인 가능
    
    ```sql
    kubectl get deployments --selector=app=dashboard -w
    ```
    
6. 배포 완려됐다면 녹색 대시보드가 뜨는지 확인



## 젠킨스 플러그인을 통해 구현되는 GitOps

젠킨스 플러그인은 사용자에게 필요한 기능을 주로 젠킨스 플러그인 홈페이지에서 검색해 내용을 살펴보고 이를 조합하는 방식을 취합니다.

제공되는 플러그인 조합

- Platforms : 웹 애플리케이션이 아닌 다른 플랫폼에서 작동하는 애플리케이션 빌드를 위한 플러그인 종류
- User interface : 젠킨스의 기본 UI 외의 확장 UI를 적용하기 위한 플러그인 카테고리
- Administration : LDAP, 젠킨스 클러스터 관리 등 젠킨스 자체관리에 필요한 플러그인 종류
- Source code management: 깃허브 및 깃랩 같은 소스 코드 저장소의 연결이나 관리를 위한 플러그인 카테고리
- Build management : CI/CD 단계에서 추가적으로 사용할 수 있는 플러그인 종류

쿠버네티스용 지속적 배포(Kubernetes Continous Deploy), 슬랙 알림(Slack Notification), 변경 내용 비교(Last Changes) 총 3가지 플러그인을 조합해 젠킨스에서 GitOps를 구현해볼 예정

GitOps: 깃을 통해 모든 것을 선언적으로 깃헙 저장소 같은 SCM에 업데이트된 오퍼레이터가 변경분을 감지해 대상 시스템에 배포함

현재 쿠버네티스 환경에 맞춰 설명하면 배포돼야 할 매니페스트 파일을 깃헙 저장소에 저장하고 매니페스트가 새로 업데이트됐다면 젠킨스가 이를 파악해 쿠버네티스 클러스터에 배포하는 모든 단계를 GitOps라고 한다.

- 사람의 실수를 줄일 수 있음 - 배포 자동화
- 깃헙 저장소의 내용을 실제 운영 환경의 내용을 동일하게 가져갈 수 있음(단일한 관리)
- 배포 표준화해 자동 배포 가능

### 쿠버네티스 환경에 적합한 선언적인 배포 환경

외부 클러스터에 접근하려면 쿠버설정(kuberconfig) 파일을 이용해 외부 클러스터의 API 서버로 접근한 다음 오브젝트를 배포해야 하는데, 이는 보안적인 문제가 있음

따라서 쿠버 설정 파일을 젠킨스 컨트롤러에서 관리하고 상황에 따라 필요한 권한만을 제공하는 기능이 필요한데 쿠버네티스용 지속적 배포 플러그인을 사용해 이를 구현할 수 있다.

1. 주기적으로 변화를 감지해야 하는 깃헙 저장소는 모두 같은 저장소를 공유할 수 없으므로 실습 사용자마다 필요함. 깃헙 저장소를 생성함
2. 저장소 생성하고 주소를 복사함. 이후 생성한 매니페스트를 푸시하기 위한 도구로 사용됨
3. GitOps의 내용을 저장할 디렉터리 생성 후 그 디렉터리로 이동하고 git init함
    ...
    // git config username, email, credential
    
    git remote add origin <깃헙 레포 주소>
    
    // 사전 구성해둔 파일을 홈 디렉터리 밑의 gitops 디렉터리로 복사
    cp ~/~~/gitops
    
    // 사전 구성된 jenkinsfile에는 이미 배포를 위한 설정이 되어 있었으나
    // 현재 레포는 개별 사용자에 맞는 설정이 필요함
    // sed s/변경 대상/변경할 값/g
    sed -i 's,Git-URL,<사용자의 깃헙 레포 주소>,g' Jenkinsfile
...
4. 쿠버설정 파일을 안전하게 관리하기 위해서 쿠버네티스용 지속적 배포 플러그인 설치(kubernetes coninuous deploy)
    1. 젠킨스 홈 화면 > 젠킨스 관리 > 플러그인 관리 > 설치 가능
    2. kubernetes continuous deploy 설치하고 재시작
5. 다시 젠킨스에 로그인해서 쿠버설정 파일 설정 메뉴로 이동
    1. 젠킨스 홈 화면 > 젠킨스 관리 > Manage Credentials
6. 쿠버네티스용 지속적 배포 플러그인이 사용할 새로운 자격 증명 정보 추가하기 위해 global 버튼 클릭
7. add credential을 눌러 마스터 노드에 대한 자격 증명을 설정함
    1. kind : 자격 증명 종류. 마스터 노드 접속 시 사용자 이름과 비밀번호 입력 받는 식으로 하기 위해 Username with Password를 선택
    2. scope : 자격 증명이 적용되는 범위를 정함. 젠킨스 전역에서 자격 증명을 사용할 수 있게 Global(Jenkins, nodes, items, all child items, etc)로 선택
    3. username : 시스템에 접속하기 위한 사용자 이름 입력. 마스터 노드에 접속하는 기본 계정인 root 입력
    4. password : 시스템 접속을 위한 사용자 비밀번호. 베이그런트로 만들어진 가상 머신들의 초기 비밀번호는 vagrant다. 
    5. id : 자격 증명을 사용할 때 식별하기 위한 값
    6. description : 자격 증명에 대한 간단한 설명을 작성 가능
8. 위의 자격 증명을 설정 후 또 add credentials을 눌러 쿠버설정 파일에 대한 자격 증명을 추가
    1. kind는 kubernetes configuration(kuberconfig) 선택
    2. scope : 젠킨스 전역에서 증명을 사용할 수 있게 global 선택
    3. kuberconfig : 쿠버 설정 파일을 가져오는 방법 설정. 마스터 노드에 존재하는 kuberconfig임을 나타내기 위해 kubeconfig get from master node를 입력
    4. Server : 쿠버설정 파일이 존재하는 서버의 IP 입력. 여기선 마스터 노드 IP 입력
    5. SSH Credentials: 마스터 노드에 접근하기 위한 자격 증명 선택. 앞서 7번에서 만든 자격 증명 선택
    6. File: 마스터 노드에 위치한 kubeconfig 위치를 설정. 경로는 .kube/config로 이미 입력돼 있음
9. 젠킨스 홈 화면으로 이동해 새로운 Item → Pipeline을 선택함
10. 깃헙 저장소 변경 내용 감시를 위해 pipeline 프로젝ㅌ에서 지원하는 기능인 poll SCM을 사용해 주기적으로 깃헙 저장소의 변경을 인식하게 한다. 
    1. 젠킨스에서 주기적으로 변경을 감시하기 위해 스케줄을 */10 * * * *로 입력함
        1. 크론 표현식으로 10분마다 변화가 있는지 체크하도록 설정
11. Pipeline script from SCM을 선택하고 레포지터리 url, 메인 브랜치 선택
12. 이 때 사용하는 Jenkinsfile은 깃헙에서 yaml 파일을 내려 받고 → 미리 설정한 kubeconfig 자격 증명을 이용해서 경로에 존재하는 yaml 파일들의 내용을 쿠버네티스 클러스터에 배포한다.
13. 선언적인 배포 환경을 테스트하기 위해 yaml 파일을 변경하고 푸시하면 배포돼 있는 디플로이먼트도 변경되는지 확인해보자
...
젠킨스에서 보통 슬랙을 가장 많이 연결해 알림 메시지를 받아봄.

간단히 연동하는 단계를 살펴보자.

1. 젠킨스가 슬랙으로 메시지를 보낼 수 있는 대상인 슬랙 채널을 생성함
2. 슬랙 채널에 젠킨스가 보내는 메시지를 전달할 수 있는 Jenkins CI 앱을 추가해 젠킨스에서 슬랙 채널 연동을 위한 토큰과 워크스페이스 도메인 주소 값을 확인한다.
3. 슬랙에서 발급한 토큰은 공개되면 매우 민감한 정보이므로 젠킨스 자격 증명에 토큰을 등록한다.
4. 젠킨스에서 슬랙으로 메시지를 보내기 위해서 슬랙 알림 플러그인을 설치하고 시스템 설정 메뉴에 토큰과 워크스페이스 도메인 주소를 입력해 연동 작업을 마친다.

1. 슬랙 새 채널 생성하고 필요한 사용자 추가
2. 젠킨스를 연동할 앱을 설치하기 위해 설정 및 관리의 하위 메뉴인 앱 관리를 선택함
    1. 슬랙 앱 관리 화면 검색창에서 Jenkins CI를 검색하고 검색된 Jenkins CI를 선택함 → Slack에 추가하고 알림을 받을 채널 선택
    2. Jenkins CI 통합 앱 추가 버튼을 눌러 슬랙에 Jenkins CI 앱을 추가함
    3. 연동 설정 지침 중 자격 증명 정보 확인 - 우리는 통합 토큰 자격 증명 ID를 직접 프로젝트에 입력해 구성하지 않고 젠킨스 자격 증명 관리를 통해 토큰을 관리할 것임
3. 통합 토큰 자격 증명 ID를 젠킨스 자격 증명 키로 등록하기 위해 자격 증명 등록 화면으로 이동
    1. 젠킨스 홈 화면 > 젠킨스 관리 > Managed Credentials > (global)
4. Add Credentials 버튼 클릭
    1. Kind - Secret text 선택
    2. 슬랙 연동 설정 지침 중 자격 증명 정보 확인에 있던 통합 토큰 자격 증명 ID를 Secret에 입력
    3. scope : global
5. 등록한 슬랙 자격 증명 암호키를 사용해서 젠킨스로부터 슬랙에 메시지를 보내려면 이를 도와줄 플러그인을 설치해야 함 
    1. 젠킨스 홈 화면 > 젠킨스 관리 > 플러그인 관리 > 설치 가능 탭으로 이동
    2. Slack Notification 선택하고 다운로드 후 재시작
6. 슬랙 알림 플러그인 설치 후, 젠킨스 홈 화면 > 젠킨스 관리 > 시스템 설정으로 이동
    1. 가장 하단에 Slack 부분에 필요한 정보 저장
        1. Workspace : 슬랙 워크스페이스 도메인 입력, 설정 지침에서 본 슬랙의 팀 하위 도메인 입력(통합 토큰 자격 증명 ID 재입력)
        2. credential : 앞서 4번에서 설정한 key 선택
        3. default channel/member id : 젠킨스로부터 온 메시지를 받을 슬랙 채널을 입력. 여기서 앞서 추가한 채널을 선택함
7. Jenkinsfile 수정
    ...
    ... // git pull(Git-URL 포함), k8s deploy ...
    
    		stage('deploy end') {
    			steps { // + 추가됨, 배포 작업 이후 배포 완료 알림 메시지를 슬랙 채널로 보냄
...
1. Jenkinsfile에 사용자 깃헙 버장소 입력을 위해 현재 변수(Git-URL)로 처리돼 잇는 부분을 sed 명령을 통해 사용자의 깃헙 주소로 변경
    
    ```bash
    sed -i 's,Git-URL, <사용자 깃헙 주소>,g' Jenkinsfile
...
슬랙을 통해 단순 배포 관련 메시지만이 아니라 코드 변경도 함께 확인할 수 있다.

1. 젠킨스 홈 화면 > 젠킨스 관리 > 플러그인 관리 > 설치 가능 탭에서 last change를 입력해 해당 플러그인 설치
    1. 설치 끝나고 실행 중인 작업이 없으면 Jenkins 재시작 체크
2. 플러그인 설치 완료 후 변경 내용을 비교하기 위해 Jenkinsfile 수정
    
    아래 내용 추가
    
    Last Changes 플러그인에서는 pipeline 문법이 적용되지 않아 Groovy 스크립트를 사용해 script 이전 배포와 현재 배포의 차이를 찾아소 html 파일로 작성하도록 구성함. 이렇게 작성된 html이 슬랙 메시지로 전달되는 링크를 통해 확인하거나 프로젝트 상세 화면읮 좌측 메뉴를 통해 확인할 수 있다.
    
    ```bash
    	stage('send diff') {
    		steps {
    			script { // +추가: 이전 배포와 현재 배포의 코드 변동 사항을 html로 만듬
    	...
    	"SIDE", "LINE", true, true, "", "", "",  "", ""
    				...
    				} // 변경 사항을 한 눈에 볼 수 있는 주소를 메시지로 전달
    				slackSend(message: """${env.JOB_NAME} #${env.BUILD_NUMBER} End
...
3. Jenkinsfile에 사용자 깃헙 버장소 입력을 위해 현재 변수(Git-URL)로 처리돼 잇는 부분을 sed 명령을 통해 사용자의 깃헙 주소로 변경